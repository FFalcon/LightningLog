...
void Options::loadSettings() {
  auto path = QCoreApplication::applicationDirPath() + "loglightning.conf";
  ...
  for (auto i = 0; i < nitems; i++) {
    ...
    listItem->setData(Qt::FontRole,
                      QFont("Cascadia Code", -1,
                            filterOptions.isBold ? QFont::Bold : QFont::Normal,
                            filterOptions.isItalic));
...
void Options::on_saveBtn_clicked() {
  ...
  int idx = 0;
  for (auto it = normalModeFilterMap->begin(); it != normalModeFilterMap->end();
       *it++, idx++) {
    ...
    value += "|";
    value += QString("caseSensitive=") +
             (it.value().isCaseInsensitive ? "true" : "false");
